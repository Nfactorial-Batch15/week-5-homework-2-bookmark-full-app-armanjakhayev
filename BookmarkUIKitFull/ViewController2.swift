...
class ViewController2: UIViewController, UIAlertViewDelegate, UITableViewDelegate, UITableViewDataSource {
...
//        let save = UIAlertAction(title: "Save", style: .cancel , handler: { _ in
//            guard let bookmarkName = alertView.textFields?[0].text,let bookmarkLink = alertView.textFields?[1].text else { return }
//            let bookmark = Bookmark(title: nametitle, link: namelink)
//            self.bookmarks.append(bookmark)
//            Storage.bookmarkModels.append(Bookmark(title: nametitle, link: namelink))
//            self.tableView.reloadData()
//        })
//        alertView.addAction(save)
//        self.present(alertView, animated: true, completion: nil)
